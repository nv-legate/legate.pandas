...
for n in [15, 30, 45]:
    categories = ["C" + str(i) for i in range(n // 3 + 1)]
    categories.sort()
    key_dtype = pd.CategoricalDtype(categories=categories)
    ...
    key_right = pd.Categorical.from_codes(list(range(n // 3)), dtype=key_dtype)
    ...
    key_left = pd.Categorical.from_codes(key_left, dtype=key_dtype)
