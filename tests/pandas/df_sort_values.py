...
for n in [15, 30, 45]:
    ...
    for ascending1 in [True, False]:
        for ascending2 in [True, False]:
            for ignore_index in [False, True]:
                print(
                    "Size: %u, Ascending: [%s,%s,%s], Ignore index: %s"
                    % (n, ascending1, ascending2, ascending2, ignore_index)
                )

                out_df = df.sort_values(
                    ["c1", "c2", "c3"],
                    ignore_index=ignore_index,
                    ascending=[ascending1, ascending2, ascending2],
                )
