...
/*static*/ void PadTask::cpu_variant(const Task *task,
                                     ...
                                     Runtime *runtime)
{
  ...
  PadSideCode side;
  {
    ...
    side = static_cast<PadSideCode>(code);
  }
    ...
    if (in_size >= width) {
      ...
      switch (side) {
        case PadSideCode::LEFT: {
          ...
          for (auto k = 0; k < pad_size; ++k) *out_chars++ = fill_char[0];
          memcpy(out_chars, in_chars, in_size);
          in_chars += in_size;
          out_chars += in_size;
          break;
        ...
        case PadSideCode::BOTH: {
          out_size            = width;
          auto pad_size       = width - in_size;
          auto left_pad_size  = pad_size / 2;
          auto right_pad_size = pad_size - left_pad_size;
          for (auto k = 0; k < left_pad_size; ++k) *out_chars++ = fill_char[0];
          memcpy(out_chars, in_chars, in_size);
          in_chars += in_size;
          out_chars += in_size;
          for (auto k = 0; k < right_pad_size; ++k) *out_chars++ = fill_char[0];
          break;
        }
      }
    }
...
static void __attribute__((constructor)) register_tasks(void) { PadTask::register_variants(); }
