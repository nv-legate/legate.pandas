...
ColumnView notna(const ColumnView &in, alloc::Allocator &allocator)
{
  auto size = in.size();
...
/*static*/ void NotNaTask::cpu_variant(const Task *task,
                                       ...
                                       Runtime *runtime)
{
  ...
  auto result = detail::notna(in.view(), allocator);
  out.return_from_view(allocator, result);
}

static void __attribute__((constructor)) register_tasks(void) { NotNaTask::register_variants(); }
